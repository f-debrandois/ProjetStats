...
Ce rapport analyse les émissions de polluants atmosphériques dans la région Occitanie sur une période de 2014 à 2019, à partir des données fournies par le site web Atmo-Occitanie. Ces données comprennent les émissions de divers polluants tels que les oxydes d'azote, le dioxyde de soufre, les particules en suspension, le monoxyde de carbone, le benzène, l'ammoniac, les gaz à effet de serre, le méthane, le dioxyde de carbone et le protoxyde d'azote, ainsi que des informations sur les EPCI (Etablissements Publics de Coopération Intercommunale) telles que leur nom, leur code d'identification, leur département d'appartenance, leur latitude, leur longitude et leur type.

Le rapport aborde plusieurs questions, allant de l'analyse descriptive des données à l'exploration des modèles prédictifs et explicatifs. Il s'intéresse notamment à la transformation des données, à la visualisation des émissions de polluants en fonction de diverses variables telles que l'année et le type d'EPCI, à la classification des EPCI en fonction de leurs émissions, à l'utilisation de l'analyse discriminante linéaire pour prédire le dépassement d'émissions de méthane et le type d'EPCI, ainsi qu'à l'explication des émissions de gaz à effet de serre et de méthane à l'aide de modèles linéaires ou généralisés, en tenant compte des interactions entre les variables. En outre, une attention particulière est accordée à la simplification des modèles et à la mise en place de régressions régularisées pour améliorer la précision des prédictions.  
...
Les critères BIC et ICL ne nous ont pas permis d’obtenir de résultats satisfaisants, et leur compilation est très longue en raison du grand nombre de composantes que nous avons dû afficher pour espérer avoir un critère d’arrêt. Nous n’avons donc pas jugé utile d’afficher les différents graphiques obtenus.
En effet,  avec un premier affichage en faisant varier le nombre de composantes de 2 à 50, aucun critère d’arrêt n’a été trouvé par le critère BIC. Cependant, le graphe nous a permis d’écarter tous les modèles sphériques et diagonaux, qui fournissent des résultats bien moins bons que les autres.

En conservant uniquement les modèles les plus « performants », on remarque que le modèle retenu dans tous les cas est VEV. Cependant, il semblerait que le critère n’arrive toujours pas à trouver un point d’arrêts pour le nombre optimal de clusters. Nous avons donc utilisé le critère ICL sur les coordonnées de l’ACP.
...
```{r fig44,echo=FALSE,eval=FALSE,fig.cap="\\label{fig:fig44}Mélanges Gaussiens avec K=5",fig.height=3}
# mélange (en janvier)
...
```{r fig45,echo=FALSE,eval=FALSE,fig.cap="\\label{fig:fig45}Mélanges Gaussiens avec K=5",fig.height=3}
...
```{r fig46,echo=FALSE,eval=FALSE,fig.cap="\\label{fig:fig46}Mélanges Gaussiens avec K=5",fig.height=3}
...
```{r fig43,echo=FALSE,eval=FALSE,fig.cap="\\label{fig:fig43}Mélanges Gaussiens avec K=5",fig.height=3}
...
Où ici, $\mu$ représente l'intercept, $\alpha_i$ représnte le type d'EPCI, $\beta_j$ l'année et $\gamma_{ij}$ représente les intéractions entre l'année et le type.
...
On obtient cette fois une p-value de $\texttt round(anova(lm(ges_teqco2 ~1, data=dlog),anov_type)$ < $0.05$.\
...
$$
 \quad \begin{cases}
  ges_i = \theta_0 + \theta_1nox_i+\theta_2so2_i + \theta_3pm10_i + \theta_4pm25_i +\theta_5co_i + \theta_6c6h6_i + \theta_7nh3_i +\theta_8 ch4_i + \theta_9co2_i + \theta_10no2_i + \varepsilon_i\\
...
```{r fig40,echo=FALSE,eval=TRUE,fig.cap="\\label{fig:fig40}Sélection de variable backward avec BIC",fig.height=3}
...
En utilisant la méthode Backward, tous les critères( ici le critère bic est le seul affiché mais tous) conduisent à la même
...
$$
 \quad \begin{cases}
  gges_i = \theta_0 + \theta_1nox_i+\theta_2so2_i + \theta_3pm10_i + \theta_4pm25_i  + \theta_5nh3_i +\theta_6 ch4_i + \theta_7co2_i + \theta_8no2_i + \varepsilon_i\\
  \varepsilon_{i} \sim \mathcal{N}(0,\sigma^2)
\end{cases}
...
```{r fig41,echo=FALSE,eval=TRUE,fig.cap="\\label{fig:fig41}Sélection de variable forward avec BIC",fig.height=3}
...
$$
 \quad \begin{cases}
  ges_i = \theta_0 + \theta_1nox_i+\theta_2so2_i + \theta_3pm10_i + \theta_4pm25_i  + \theta_5nh3_i +\theta_6 ch4_i + \theta_7co2_i + \theta_8no2_i + \varepsilon_i\\
  \varepsilon_{i} \sim \mathcal{N}(0,\sigma^2)
\end{cases}
...
Dans cette analyse, nous chercherons à expliquer l'émission de méthane en fonction de l'ammoniac, du protoxyde d'azote, du type d'EPCI et de l'année, en prenant en compte les interactions entre ces variables. Pour ce faire, nous utiliserons une Analyse de la Covariance (ANCOVA), qui nous permettra d'examiner comment les émissions de méthane varient selon les niveaux des différentes variables, tout en tenant compte des effets de l'année comme une variable continue.
...
$$
\begin{cases}
  \text{ch4}_i = \theta_0 + \theta_1  \text{nh3}_i + \theta_2  \text{n2o}_i + \theta_3  \text{TypeEPCICC}_i + \theta_4  \text{TypeEPCICU}_i + \theta_5  \text{TypeEPCIMetropole}_i + \theta_6  \text{annee}_i + \theta_7  \text{nh3}_i  \text{n2o}_i + \theta_8  \text{nh3}_i\text{TypeEPCICC}_i\\ + \theta_9  \text{nh3}_i  \text{TypeEPCICU}_i + \theta_{10}  \text{nh3}_i  \text{TypeEPCIMetropole}_i \varepsilon_{i}\\
  \varepsilon_{i} \sim \mathcal{N}(0,\sigma^2)
\end{cases}
$$
...
Les résultats obtenus sur les différentes régressions réalisées nous ont permis de comprendre que simplifier les modèles étaient assez dur, car le gaz à effet de serre dépend plus ou moins de toutes les variables.
